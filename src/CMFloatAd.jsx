...
function CMFloatAd({color = '#ffffff', bgColor = 'transparent'}) {
    ...
    const bottomLabelStyle = {
        ...
        bottom: '10px',
        right: '20px',
        // Collapsed: wide enough for the logo (85px) + padding; border-box safe
        maxWidth: '109px',
        width: '109px',
        height: isMobile && isHovered ? 'auto' : '56px',
        minHeight: isMobile && isHovered ? '80px' : '56px',
        padding: '8px 12px',
        ...
        overflow: 'hidden',
        transition: 'max-width 0.6s ease-in-out, width 0.6s ease-in-out, height 0.6s ease-in-out',
        display: 'flex',
      ...
      const hoverStyle = isHovered ? {
        maxWidth: isMobile ? 'calc(100vw - 40px)' : '680px',
        width: isMobile ? 'calc(100vw - 40px)' : '680px',
        zIndex: 1000,
